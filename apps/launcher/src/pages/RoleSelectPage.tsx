...
export function RoleSelectPage(): JSX.Element {
  ...
  const systemId = isSingleSystemEdition()
    ? (lockedId ?? queryId ?? persistedSystemId)
    : (queryId ?? lockedId ?? persistedSystemId);
  ...
  function choose(role: PopsRole): void {
    if (systemId) setSystem(systemId);
    window.location.assign(loginPathForRole(role, systemId));
  }

  return (
    <div className="min-h-screen bg-[var(--bg)] px-6 py-10 dark:bg-slate-950">
