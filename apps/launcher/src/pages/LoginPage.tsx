...
export function LoginPage(): JSX.Element {
  ...
  const queryId = querySystem && isBusinessSystemId(querySystem) ? querySystem : null;
  const systemId = isSingleSystemEdition()
    ? (systemLock ?? queryId ?? persistedSystemId)
    : (queryId ?? systemLock ?? persistedSystemId);
  ...
  if (!systemReady && !isSuperAdminLogin && !systemId) {
    return <div className="mx-auto flex min-h-screen max-w-md items-center justify-center text-sm text-slate-500">Loading…</div>;
  }
  ...
  async function completeLogin(accessToken: string, refreshToken: string): Promise<void> {
    ...
    if (assignedSystemId && systemId && !assignedSystemAllows(assignedSystemId, systemId)) {
      clearSession();
      throw new Error(
  ...
  if (isSuperAdminLogin && roleMeta) {
    return (
      <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center bg-[var(--bg)] px-6 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900">
        ...
        <div className="mb-3 rounded-lg border border-[color:var(--brand)]/25 bg-[var(--brand-cream)] px-3 py-2 text-center text-xs font-semibold text-[color:var(--ink)]">
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center bg-[var(--bg)] px-6 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900">
      ...
      <div className={`${loginCardClass} shadow-lg ${accentRing}`}>
        ...
        <h1 className="mt-1 text-lg font-semibold text-[color:var(--ink)]">
          {roleMeta!.label}
        </h1>
        <p className={`mt-1 text-sm ${mutedClass}`}>{roleMeta!.description}</p>
