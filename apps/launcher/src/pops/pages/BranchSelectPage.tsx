...
export function BranchSelectPage(): JSX.Element {
  ...
  const [selected, setSelected] = useState<PopsBranch | null>(null);
  ...
  useEffect(() => {
    ...
    const stillValid = selected && allBranches.some((b) => b.id === selected.id);
    if (stillValid) return;
    const preferred =
      (persistedBranch && allBranches.find((b) => b.code === persistedBranch.code)) ??
      allBranches.find((b) => !isMonitoringBranch(b.code)) ??
      allBranches[0] ??
      null;
    setSelected(preferred);
  }, [allBranches, selected, persistedBranch]);
  ...
  function openErp(branch: PopsBranch | null): void {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-6 py-10 text-slate-100">
      <div className="mx-auto max-w-3xl">
              ...
              <div className="mt-4 space-y-2">
                {allBranches.map((b) => (
                  <button
                    ...
                    type="button"
                    onClick={() => openErp(b)}
                    className={`flex w-full cursor-pointer items-center justify-between gap-2 rounded-lg border px-3 py-3 text-left text-sm transition ${
                      selected?.id === b.id
        ...
        <div className="sticky bottom-0 z-20 mt-8 flex flex-col items-center gap-3 border-t border-slate-200 bg-slate-50/95 py-4 dark:border-slate-800 dark:bg-slate-950/95">
          <Button
            type="button"
            className="min-w-[14rem] !bg-amber-500 !text-slate-950 hover:!bg-amber-400"
          ...
          <p className="text-xs text-slate-500">Or click a branch above to open it.</p>
        </div>
      </div>
    </div>
  );
