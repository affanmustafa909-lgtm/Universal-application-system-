...
import { posCategoryEmoji, scopeMenuForBusinessSystem } from "../../lib/posCategoryVisual";
...
const POS_PRIMARY_ORDER_BTN = `${POS_ACTION_BTN} h-10 border-0 bg-[var(--brand)] text-[color:var(--brand-fg,var(--ink))] shadow-sm hover:bg-[var(--brand-dark)]`;
const POS_PRIMARY_PAY_BTN = `${POS_ACTION_BTN} h-10 border-0 bg-[var(--brand-yellow)] text-[color:var(--brand-yellow-fg,var(--ink))] shadow-sm hover:brightness-95`;
const POS_SECONDARY_BTN = `${POS_ACTION_BTN} h-9 border border-[color:var(--pos-accent,var(--line))] bg-white font-medium text-[color:var(--ink)] hover:border-[color:var(--pos-accent,var(--brand))] hover:bg-[var(--brand-cream)] dark:border-slate-700/80 dark:bg-slate-900/50 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:bg-slate-800 dark:hover:text-white`;
const POS_TOOLBAR_BTN =
  "inline-flex shrink-0 items-center justify-center rounded-lg px-2 py-1.5 text-[10px] font-semibold text-[color:var(--ink)] transition hover:bg-[var(--brand-cream)] hover:text-[color:var(--brand-dark)] dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white";
const POS_KEYS_BTN =
  "inline-flex shrink-0 items-center justify-center rounded-lg border border-[color:var(--line)] bg-white px-2 py-1.5 text-[10px] font-semibold text-[color:var(--ink)] transition hover:bg-[var(--brand-cream)] dark:border-slate-600/80 dark:bg-transparent dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white";
const POS_HEADER_TOGGLE_BTN =
  "inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[color:var(--line)] bg-white text-[color:var(--muted)] transition hover:bg-[var(--brand-cream)] hover:text-[color:var(--ink)] dark:border-slate-600/80 dark:bg-transparent dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white";
const POS_MODE_BAR =
  "no-scrollbar flex shrink-0 gap-1 overflow-x-auto rounded-lg bg-[var(--brand-cream)] p-1 ring-1 ring-[color:var(--line)] dark:bg-slate-950/80 dark:ring-slate-800/80";
const POS_MODE_BTN = (active: boolean) =>
  `shrink-0 whitespace-nowrap rounded-md px-3 py-2 text-xs font-semibold transition ${
    active
      ...
      : "text-[color:var(--muted)] hover:bg-white hover:text-[color:var(--ink)] dark:text-slate-400 dark:hover:text-slate-200"
  }`;

function readPosCategoryLayout(isIceCream: boolean): "list" | "icon" {
  try {
    const key = isIceCream ? "pops-pos-category-layout-icecream" : "pops-pos-category-layout-restaurant";
    const raw = localStorage.getItem(key) ?? localStorage.getItem("pops-pos-category-layout");
    if (raw === "list" || raw === "icon") return raw;
  } catch {
    /* ignore */
  }
  return isIceCream ? "icon" : "list";
}

function persistPosCategoryLayout(isIceCream: boolean, layout: "list" | "icon"): void {
  try {
    const key = isIceCream ? "pops-pos-category-layout-icecream" : "pops-pos-category-layout-restaurant";
...
export function PosPage(): JSX.Element {
  ...
  const [categoryLayout, setCategoryLayout] = useState<"list" | "icon">(() =>
    readPosCategoryLayout(isIceCream),
  );
  useEffect(() => {
    setCategoryLayout(readPosCategoryLayout(isIceCream));
  }, [isIceCream]);
  ...
  const { categories, items: menuItems } = useMemo(
    () =>
      scopeMenuForBusinessSystem(
        systemId,
        menuQuery.data?.categories ?? [],
        menuQuery.data?.items ?? [],
      ),
    [systemId, menuQuery.data],
  );
  ...
  function invalidateOrderFeeds(): void {
    ...
    if (!branch?.code) return;
    void queryClient.refetchQueries({ queryKey: ["kitchen", branch.code] });
    void queryClient.refetchQueries({ queryKey: ["orders", branch.code] });
  }

  function rememberKitchenTicket(ticket: KitchenTicket): void {
    if (!branch?.code) return;
    queryClient.setQueryData<KitchenTicket[]>(["kitchen", branch.code], (prev) => {
      const list = prev ?? [];
      const idx = list.findIndex(
        (row) => row.id === ticket.id || (ticket.orderRef && row.orderRef === ticket.orderRef),
      );
      if (idx >= 0) {
        const next = [...list];
        next[idx] = ticket;
  ...
  function rememberBill(bill: Bill): void {
    if (!branch?.code) return;
    queryClient.setQueryData<Bill[]>(["orders", branch.code], (prev) => {
      const list = prev ?? [];
      const idx = list.findIndex((row) => row.id === bill.id);
      if (idx >= 0) {
        const next = [...list];
        next[idx] = bill;
        return next;
  ...
  const createOrderMutation = useMutation({
    ...
    onSuccess: async (ticket) => {
      ...
      rememberKitchenTicket(ticket);
      invalidateOrderFeeds();
      let printed = { errors: [] as string[], skippedNoChanges: false as boolean | undefined };
      try {
        // Silent-print after the sidebar updates so a printer dialog cannot hide the new order.
        printed = await printKitchenKotsOnPay(ticket.orderRef ?? orderRef);
      } catch (printErr) {
        printed = {
          errors: [printErr instanceof Error ? printErr.message : String(printErr)],
        };
      }
      ...
      if (!wasTicketEdit) {
        try {
          ...
          // Order is already saved — do not block the ticket reset on HR logging.
        }
      }
  ...
  return (
    <div className="flex min-h-[calc(100vh-4.25rem)] flex-col gap-2">
      ...
      {/* Compact toolbar */}
      <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-2.5 py-2 shadow-card dark:border-slate-800/80 dark:bg-slate-900/40">
        <button
          type="button"
        ...
        <div className="shrink-0 text-sm font-semibold text-[var(--ink)] dark:text-white">POS</div>
        <div ref={searchContainerRef} className="relative min-w-0 flex-1 basis-[10rem] sm:max-w-md">
          <input
            ref={searchInputRef}
            placeholder={isIceCream ? "Search menu or scan SKU... (F9)" : "Search menu or scan SKU"}
            value={search}
            onChange={(e) => {
            ...
            className="w-full rounded-2xl border border-[var(--line)] bg-[var(--brand-cream)] px-4 py-2.5 text-sm text-[var(--ink)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--brand)] focus:ring-1 focus:ring-[var(--brand)]/25 dark:border-slate-700/80 dark:bg-slate-950 dark:text-white"
          />
          {searchDropdownOpen && isSearching && searchDropdownItems.length > 0 ? (
            <div className="absolute left-0 top-full z-30 mt-1 w-full overflow-hidden rounded-lg border border-[var(--line)] bg-white shadow-panel dark:border-slate-700 dark:bg-slate-900">
                  ...
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-2 px-2.5 py-1.5 text-left text-xs transition ${
                      index === searchHighlight
                        ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]"
                        : "text-[var(--ink)] hover:bg-[var(--brand-cream)] dark:text-slate-200 dark:hover:bg-slate-800"
                    }`}
                  >
        ...
        <div className="flex w-full shrink-0 flex-wrap items-center gap-1 sm:ml-auto sm:w-auto">
          {cashSessionQuery.data ? (
            <span className="rounded-md bg-[#C2E8C8] px-2 py-1 text-[10px] font-semibold text-[var(--ink)] ring-1 ring-[var(--muted)]/20">
              Shift open · {cashSessionQuery.data.sessionRef}
            </span>
          ) : (
            <span className="rounded-md bg-[var(--brand-yellow)] px-2 py-1 text-[10px] font-semibold text-[var(--ink)] ring-1 ring-[var(--muted)]/20">
          ...
          <button
            ...
            className={`${POS_TOOLBAR_BTN}${
              deliveryDetailsOpen && posModeShowsCustomerPanel(mode)
                ? " bg-[var(--brand-cream)] text-[var(--brand-dark)] dark:bg-slate-800 dark:text-white"
                : ""
            }`}
          ...
          <div className="ml-1 flex items-center gap-1.5 border-l border-[var(--line)] pl-2 dark:border-slate-700/80">
            <details className="relative">
              ...
              <div className="absolute right-0 z-40 mt-1 w-56 rounded-lg border border-[var(--line)] bg-white p-2 text-[10px] text-[var(--ink)] shadow-panel dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
                {(Object.keys(POS_SHORTCUTS) as (keyof typeof POS_SHORTCUTS)[]).map((id) => (
                  <div key={id} className="flex justify-between gap-2 py-0.5">
                    ...
                    <kbd className="rounded bg-[var(--brand-cream)] px-1 font-mono text-[var(--brand-dark)] dark:bg-slate-800 dark:text-amber-300">
                      {POS_SHORTCUTS[id].key}
                    </kbd>
                  </div>
                ))}
                <div className="mt-1 border-t border-[var(--line)] pt-1 text-[var(--muted)] dark:border-slate-800">
                  Qty + also works with <kbd className="font-mono text-[var(--brand-dark)]">+</kbd>
                </div>
              </div>
            </details>
          </div>
      ...
      <div className="grid flex-1 grid-cols-12 gap-3 lg:items-start">
        ...
        <div className={`col-span-12 flex min-h-0 flex-col lg:sticky lg:top-0 lg:h-[calc(100vh-9rem)] lg:max-h-[calc(100vh-9rem)] ${isIceCream ? "lg:col-span-5" : "lg:col-span-4"}`}>
                ...
                <div className="flex items-center gap-1.5">
                  {posSettings.fullScreenMenuEnabled ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-md bg-[var(--brand)] px-2 py-1 text-[10px] font-bold text-[color:var(--brand-fg,var(--ink))] shadow-sm hover:bg-[var(--brand-dark)]"
                    >
                  ...
                  <div
                    className="inline-flex rounded-md border border-[var(--line)] bg-white p-0.5 dark:border-slate-700 dark:bg-slate-950/80"
                    role="group"
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      persistPosCategoryLayout(isIceCream, "list");
                    }}
                    className={`rounded px-1.5 py-1 transition ${
                      categoryLayout === "list"
                        ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]"
                        : "text-[var(--muted)] hover:text-[var(--ink)] dark:text-slate-400 dark:hover:text-white"
                    }`}
                  >
                    <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      persistPosCategoryLayout(isIceCream, "icon");
                    }}
                    className={`rounded px-1.5 py-1 transition ${
                      categoryLayout === "icon"
                        ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]"
              ...
              {categoryLayout === "list" ? (
                <div className="flex max-h-36 flex-col gap-1 overflow-y-auto pr-0.5">
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-left text-[11px] font-semibold transition ${
                      showAllItems
                        ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))] shadow-sm"
                ...
                <div className="no-scrollbar flex max-h-none gap-2 overflow-x-auto pb-0.5">
                  <button
                    type="button"
                    onClick={() => {
                      setMenuView("all");
                  ...
                  >
                    <span
                      className={`flex h-10 w-10 items-center justify-center rounded-xl text-lg ${
                        showAllItems ? "bg-white/30" : "bg-[var(--brand-cream)]"
                      }`}
                    >
                      {posCategoryEmoji("all")}
                    </span>
                  ...
                  >
                    <span
                      className={`flex h-10 w-10 items-center justify-center rounded-xl text-lg ${
                        showFeaturedOnly ? "bg-white/30" : "bg-[var(--brand-cream)]"
                      }`}
                      aria-hidden
                    >
                      {posCategoryEmoji("featured")}
                    </span>
                    ...
                    return (
                      ...
                      >
                        {img ? (
                          ...
                          <span
                            className={`flex h-10 w-10 items-center justify-center rounded-xl text-lg ${
                              active ? "bg-white/30" : "bg-[var(--brand-cream)]"
                            }`}
                          >
                            {posCategoryEmoji(c.name) === "🍽️" ? initial : posCategoryEmoji(c.name)}
                          </span>
                        )}
                        <span className="line-clamp-2 w-full text-[10px] font-semibold leading-tight">
                          {c.name}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pr-0.5">
            {menuQuery.isLoading ? (
              ...
              <div className={isIceCream ? "grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4" : "grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4"}>
                    ...
                    >
                      {img ? (
                        <img
                          src={img}
                          alt=""
                          className={`mb-1.5 w-full rounded-xl object-cover ${isIceCream ? "h-24" : "h-16"}`}
                        />
                      ) : (
                        <div
                          className={`mb-1.5 flex items-center justify-center rounded-xl bg-[var(--brand-cream)] dark:bg-slate-950/60 ${
                            isIceCream ? "h-24 text-[20px]" : "h-16 text-[16px]"
                          }`}
                        >
                          {posCategoryEmoji(categoryById.get(item.categoryId) ?? "")}
                        </div>
                      )}
                      <span className="line-clamp-2 text-[12px] font-semibold leading-tight text-[color:var(--ink)] dark:text-slate-100">
                        {item.featured ? (
                          <span className="mr-0.5 text-[color:var(--brand)]" aria-hidden>
                            ★
                          </span>
                        ) : null}
                        {item.name}
                      ...
                      {isIceCream ? (
                        <span className="mt-0.5 truncate text-[8px] font-medium uppercase tracking-wide text-[color:var(--muted)]">
                          {categoryById.get(item.categoryId) ?? "Menu"}
                        </span>
                      ) : null}
                      <div className="mt-1 flex items-end justify-between gap-1">
                        <span className="text-[13px] font-bold tabular-nums text-[color:var(--brand-dark)]">
                          {hasPicker ? "From " : ""}
                          {isIceCream ? "Rs " : ""}
                          {displayPrice.toLocaleString()}
                          {showHappyHourPrice ? (
                            <span className="ml-1 font-normal text-[color:var(--muted)] line-through">
                      ...
                      {item.featured || item.barcode || happyHourGiftItemIds.has(item.id) ? (
                        <div className="mt-0.5 flex flex-wrap items-center gap-0.5">
                          ...
                          {item.barcode ? (
                            <span className="truncate text-[8px] text-[var(--muted)]">{item.barcode}</span>
                          ) : null}
                        </div>
                      ) : null}
                    </button>
        ...
        <div className="col-span-12 flex min-h-[36rem] flex-col rounded-2xl border border-[var(--line)] bg-white shadow-card lg:col-span-4 lg:sticky lg:top-0 lg:h-[calc(100vh-9rem)] lg:max-h-[calc(100vh-9rem)] lg:min-h-0 dark:border-slate-700/50 dark:bg-gradient-to-b dark:from-slate-900/95 dark:to-slate-950 dark:shadow-xl dark:shadow-black/25 dark:ring-1 dark:ring-white/5">
          <div className="shrink-0 border-b border-[var(--line)] bg-[var(--brand-cream)] px-3 py-2.5 dark:border-slate-800/80 dark:bg-slate-900/40 dark:backdrop-blur-sm">
            <div className="flex items-center justify-between gap-2">
              <div>
                <div className="flex items-center gap-2">
                  <div className="text-[10px] font-medium uppercase tracking-wider text-slate-500">
                    {editingOrder ? "Editing" : isIceCream ? "Current order" : "Current ticket"}
                  </div>
                  <span className="rounded-full bg-[var(--brand)] px-2 py-0.5 text-[10px] font-semibold text-[color:var(--brand-fg,var(--ink))]">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-lg bg-white px-2.5 py-1 font-mono text-sm font-bold tracking-wide text-[var(--ink)] ring-1 ring-[var(--line)] dark:bg-amber-500/10 dark:text-amber-400 dark:ring-amber-500/25">
                  {orderRef}
                </span>
              </div>
            </div>
          ...
          <div className="mt-auto shrink-0 border-t border-slate-200 bg-slate-50 p-3 shadow-[0_-4px_12px_rgba(15,23,42,0.06)] dark:border-slate-800/80 dark:bg-slate-950/95 dark:shadow-[0_-4px_12px_rgba(0,0,0,0.35)]">
            <div className="rounded-lg bg-white p-3 ring-1 ring-slate-200 dark:bg-slate-950/70 dark:ring-slate-800/80">
              ...
              <div className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3 dark:border-slate-800">
                ...
                <span className="text-xl font-bold tabular-nums text-[var(--brand-dark)] dark:text-amber-400">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                onClick={() => onPay()}
              >
                {checkoutMutation.isPending ? "…" : isIceCream ? "Pay Now" : "Pay"}
              </button>
            </div>
        ...
        <div className={`col-span-12 flex min-h-[18rem] flex-col lg:sticky lg:top-0 lg:h-[calc(100vh-9rem)] lg:max-h-[calc(100vh-9rem)] ${isIceCream ? "lg:col-span-3" : "lg:col-span-4"}`}>
          <PosLatestOrdersPanel
            orders={recentOrders}
            isLoading={kitchenQuery.isLoading || ordersQuery.isLoading}
            isError={
              (kitchenQuery.isError || ordersQuery.isError) && recentOrders.length === 0
            }
