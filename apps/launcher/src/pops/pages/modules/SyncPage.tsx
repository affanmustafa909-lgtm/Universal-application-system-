...
export function SyncPage(): JSX.Element {
  ...
  function onModeChange(mode: DataMode): void {
    setDataMode(mode);
    setNotice(mode === "cloud" ? "Cloud mode — new data goes to the local Postgres database." : "Local mode — data stays on this device until you sync.");
    setError(null);
  }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle="Cloud mode updates the local Postgres database. Device-only mode keeps sales on this computer until you push."
        actions={
          <Button
            className="text-xs"
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-4 space-y-4">
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-700/80 bg-slate-950/50 p-3">
            ...
            />
            <span>
              <span className="block text-sm font-medium text-white">Local database</span>
              <span className="block text-xs text-slate-400">
                Sales and changes go to the local API / PostgreSQL. Temporary offline sales auto-sync when the API is back.
              </span>
            </span>
          </label>
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-700/80 bg-slate-950/50 p-3">
            ...
            <span>
              ...
              <span className="block text-xs text-slate-400">
                Data is stored on this computer. Use &quot;Push to cloud&quot; when you want to upload to local Postgres.
              </span>
            </span>
          </label>
        </div>
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-4 space-y-3">
          ...
          <p className="text-[11px] text-slate-500">
            Cloud mode uses the local API shown above.
          </p>
        </div>
      </div>
    </div>
