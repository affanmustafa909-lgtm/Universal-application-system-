...
export function PosLatestOrdersPanel({
  ...
  const isIceCream = useActiveSystemId() === "ice-cream-bar";
  ...
  return (
    <>
      <aside className="flex h-full min-h-0 flex-col rounded-xl border border-[var(--line)] bg-white shadow-card dark:border-slate-800/80 dark:bg-slate-900/50">
        <div className="shrink-0 border-b border-[var(--line)] px-3 py-2.5 dark:border-slate-800">
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className="text-[11px] font-semibold text-[var(--ink)] dark:text-slate-200">
                {isIceCream ? "Recent orders" : "Latest orders"}
              </div>
              <div className="mt-0.5 text-[10px] text-[var(--muted)]">
                Select bill → <kbd className="rounded bg-[var(--brand-cream)] px-1 text-[var(--brand-dark)] dark:bg-slate-800 dark:text-amber-300">P</kbd> quick
                print · Close = PRA
              </div>
            </div>
            <div className="flex shrink-0 items-center gap-1.5">
              <div
                className="inline-flex rounded-md border border-[var(--line)] bg-[var(--brand-cream)] p-0.5 dark:border-slate-700/80 dark:bg-slate-950/80"
                role="group"
              ...
              {showViewAllOrders ? (
                <Link
                  to="../orders"
                  className="shrink-0 text-[10px] font-medium text-[var(--brand-dark)] hover:text-[var(--brand)]"
                >
                  View all
                </Link>
              ) : null}
          ...
          <div className="relative mt-2">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[var(--line)] bg-[var(--brand-cream)] py-1.5 pl-8 pr-2 text-[11px] text-[var(--ink)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--brand)] focus:ring-1 focus:ring-[var(--brand)]/25 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
            {isSearching ? (
              <button
                type="button"
                onClick={() => setSearch("")}
                className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded px-1 text-[10px] text-[var(--muted)] hover:text-[var(--ink)] dark:hover:text-white"
          ...
          <div className="no-scrollbar mt-2 flex gap-1 overflow-x-auto rounded-lg border border-[var(--line)] bg-[var(--brand-cream)] p-0.5 dark:border-slate-800">
            <button
              type="button"
              onClick={(e) => {
                setModeFilter("all");
              ...
              className={`shrink-0 whitespace-nowrap rounded px-2 py-1 text-[10px] font-medium transition ${
                modeFilter === "all" ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]" : "text-[var(--muted)] hover:bg-white hover:text-[var(--ink)]"
              }`}
            >
              All
            </button>
            {visibleFilterModes.map(({ label }) => (
              <button
                ...
                className={`shrink-0 whitespace-nowrap rounded px-2 py-1 text-[10px] font-medium transition ${
                  modeFilter === label ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]" : "text-[var(--muted)] hover:bg-white hover:text-[var(--ink)]"
                }`}
              >
                {label}
              </button>
            ...
            <button
              ...
              className={`shrink-0 whitespace-nowrap rounded px-2 py-1 text-[10px] font-medium transition ${
                modeFilter === "Paid" ? "bg-[var(--pos-accent,var(--brand))] text-[color:var(--pos-accent-fg,var(--brand-fg,var(--ink)))]" : "text-[var(--muted)] hover:bg-white hover:text-[var(--ink)]"
              }`}
            >
              Paid
            </button>
                    ...
                    <li key={order.id}>
                      <div
                        ...
                        className={[
                          ...
                          isSelected
                            ? "border-[var(--brand)] bg-[var(--brand-cream)] ring-1 ring-[var(--brand)]/40 dark:border-amber-500/50 dark:bg-slate-900 dark:ring-amber-500/20"
                            : "border-[var(--line)] bg-white hover:border-[var(--brand)] dark:border-slate-800/70 dark:bg-slate-950/40 dark:hover:border-slate-700",
                        ].join(" ")}
                      >
