...
export function PosCheckoutModal({
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-xl border border-[var(--line)] bg-white shadow-panel dark:border-slate-700 dark:bg-slate-900"
        role="dialog"
        aria-modal="true"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="border-b border-[var(--line)] bg-[var(--brand-cream)] px-4 py-3 dark:border-slate-800 dark:bg-transparent">
          <h2 className="text-sm font-semibold text-[var(--ink)] dark:text-white">{title}</h2>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-3 text-xs">
          ...
          <div className="space-y-1 rounded-lg border border-[var(--line)] bg-[var(--brand-cream)] p-3 dark:border-slate-800 dark:bg-slate-950/50">
            ...
            <div className="flex justify-between border-t border-[var(--line)] pt-2 text-sm font-semibold text-[var(--ink)] dark:border-slate-800 dark:text-white">
              <span>Total</span>
              <span className="tabular-nums text-[var(--brand-dark)]">Rs {totals.total.toLocaleString()}</span>
            </div>
          </div>
          ...
          {!isHold ? (
            <div>
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[11px] font-medium text-[var(--ink)] dark:text-slate-300">Payment methods</div>
              ...
              <ul className="space-y-2">
                {payments.map((row, index) => (
                  <li key={index} className="grid grid-cols-12 gap-2">
                    <select
                      className="col-span-5 rounded-lg border border-[var(--line)] bg-white px-2 py-1.5 text-xs text-[var(--ink)] dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                      value={row.method}
                      onChange={(e) =>
                    ...
                    <input
                      type="number"
                      min={0}
                      className="col-span-5 rounded-lg border border-[var(--line)] bg-white px-2 py-1.5 text-right text-xs text-[var(--ink)] dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                      value={row.amount || ""}
              ...
              <button
                type="button"
                className="mt-2 text-[11px] font-semibold text-[var(--brand-dark)] hover:text-[var(--brand)]"
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-[var(--line)] bg-[var(--brand-cream)] p-3 dark:border-slate-800 dark:bg-transparent">
