...
export function SuperAdminBusinessDetailPage(): JSX.Element {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: async (saved) => {
      ...
      if (!applied) {
        setMessage(
          "Tax settings were not applied by the server. Redeploy backend-system, then try again.",
        );
      } else {
        setMessage(
  ...
  const saveTaxMut = useMutation({
    ...
    onSuccess: async (saved) => {
      ...
      if (!applied) {
        setMessage(
          "Tax settings were not applied by the server. Hosted API is outdated — redeploy backend-system.",
        );
      } else {
        setMessage(
