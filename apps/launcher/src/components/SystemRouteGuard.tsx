...
export function SystemRouteGuard(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const persisted = useSystemStore.getState().systemId;
    if (assignedSystemId === "restaurant" && persisted === "ice-cream-bar") {
      recordDeviceInstall(claims?.systemType ?? assignedSystemId, "ice-cream-bar");
      return;
    }
    setSystem(assignedSystemId);
    // Covers sessions restored from storage that predate the device install.
    recordDeviceInstall(claims?.systemType ?? assignedSystemId, assignedSystemId);
  ...
  if (effectiveLock) {
    ...
    const restaurantLeak =
      !isRestaurantFamilySystem(effectiveLock) && isRestaurantExclusivePath(pathname);
  ...
  if (!isRestaurantFamilySystem(systemId) && isRestaurantExclusivePath(pathname)) {
