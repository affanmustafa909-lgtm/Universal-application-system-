...
export function DistributionReportCenterPage(): JSX.Element {
  ...
  const report = useQuery({
    ...
    // Reports with a dedicated screen are never fetched here.
    enabled: Boolean(selected?.live) && !selected?.to,
    queryFn: () =>
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] min-h-0 flex-col gap-3 overflow-hidden">
      <div className="shrink-0 space-y-3">
        <PageHeader title="Report center" subtitle="All catalog reports run live — filter, export, and print." />
                      ...
                      <li key={r.id}>
                        <button
                          ...
                          className={`w-full rounded-md px-2 py-1.5 text-left text-sm ${
                            selectedId === r.id
                              ? "bg-cyan-50 font-medium text-cyan-900 dark:bg-cyan-950/40 dark:text-cyan-200"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4">
            {selected.to ? (
              <div className="space-y-3">
                ...
                <Link
                  to={selected.to}
                  className="inline-block rounded-md bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-700"
                >
                  Open {selected.title}
                </Link>
              </div>
